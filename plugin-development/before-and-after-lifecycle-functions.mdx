...
<Note>
For detailed information about these types and their definitions, check out the [code-gen-types.ts](https://github.com/amplication/amplication/blob/master/libs/util/code-gen-types/src/code-gen-types.ts) file in the Amplication GitHub repository.
</Note>
...
<Accordion title="Strategic Use of `skipDefaultBehavior`" icon="sign-ban" iconType="solid">
The `skipDefaultBehavior` option (available in some events) should be used judiciously within `before` functions. Only utilize it if you intend to completely **override** Amplication's default code generation for that specific event. In most cases, you'll want to *extend* or *modify* the default behavior, not replace it entirely.
</Accordion>
